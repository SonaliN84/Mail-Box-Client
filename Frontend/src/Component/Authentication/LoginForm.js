...
const LoginForm = () => {
  ...
  const submitHandler = (event) => {
    ...
    // let url =
    //   "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAVbpTtBs1KE9wlXJoOhNny0ZNdGYBwIEY";
    // fetch(url, {
    //   method: "POST",
    //   body: JSON.stringify({
    //     email: enteredEmail,
    ...
    //     axios
    //       .get(
    //         `https://mail-box-client-18272-default-rtdb.firebaseio.com/sent${newEmail}.json`
    //       )
    //       .then((response) => {
    //         let array = [];
    //         Object.keys(response.data).forEach((key) => {
